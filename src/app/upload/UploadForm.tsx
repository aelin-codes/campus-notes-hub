...
import { uploadNoteAction } from "@/app/actions/uploadNote";
...
export default function UploadForm({ userEmail }: { userEmail?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload file directly to Supabase storage with active client session
      ...
      // Submit via Server Action which re-checks session independently and assigns uploader_id
      ...
      await uploadNoteAction(formData);
    ...
    <div className="max-w-2xl mx-auto py-4 sm:py-8">
      ...
      <div className="mb-6 sm:mb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          {userEmail && (
            <div className="text-[11px] text-zinc-400 font-mono bg-[#1E2330] border border-[#262B38] px-2.5 py-1 rounded-lg shrink-0 self-start sm:self-auto">
              Uploading as: <span className="text-[#00E0C6]">{userEmail.split("@")[0]}</span>
            </div>
          )}
        </div>
      </div>
