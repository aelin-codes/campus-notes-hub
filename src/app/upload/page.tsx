...
export default function UploadPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 4. Insert row into notes table
      const { data: noteRow, error: dbError } = await supabase
        .from("notes")
        .insert([
          {
            title: title.trim(),
  ...
  return (
    <div className="max-w-3xl mx-auto py-4 sm:py-8">
      <div className="mb-6 sm:mb-8">
        <Link
          href="/browse"
          className="inline-flex items-center text-sm font-medium text-zinc-500 hover:text-indigo-600 mb-3"
        >
      ...
      {errorMessage && (
        <div className="mb-6 rounded-xl bg-red-50 border border-red-200 p-4 text-sm text-red-800 flex items-start">
      ...
      {successMessage && (
        <div className="mb-6 rounded-xl bg-emerald-50 border border-emerald-200 p-4 text-sm text-emerald-800 flex items-start">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="mt-2 block w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-zinc-900 placeholder-zinc-400 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none text-sm transition-all"
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div>
            <label htmlFor="department" className="block text-sm font-semibold text-zinc-800">
            ...
            <select
              ...
              className="mt-2 block w-full rounded-xl border border-zinc-300 bg-white px-3 py-2.5 text-zinc-900 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none text-sm transition-all"
            >
            ...
            {department === "Other" && (
              <input
                ...
                className="mt-2 block w-full rounded-xl border border-zinc-300 px-3 py-2 text-zinc-900 placeholder-zinc-400 focus:border-indigo-600 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none text-sm"
              />
            )}
          </div>

          <div>
            <label htmlFor="semester" className="block text-sm font-semibold text-zinc-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div>
            <label htmlFor="uploader" className="block text-sm font-semibold text-zinc-800">
        ...
        <div>
          <label className="block text-sm font-semibold text-zinc-800 mb-2">
            ...
            <span className="text-xs text-zinc-500 font-normal ml-2">(PDF or Image: PNG, JPG, WEBP)</span>
          </label>
          <div className="flex justify-center px-6 pt-5 pb-6 border-2 border-dashed border-zinc-300 rounded-2xl hover:border-indigo-500 transition-colors bg-zinc-50/60">
            <div className="space-y-2 text-center">
              ...
              {file ? (
                <div className="mt-2 inline-flex items-center px-3 py-1.5 rounded-lg bg-indigo-50 text-indigo-700 text-xs font-medium">
                ...
                <p className="text-xs text-zinc-500">PDF, PNG, JPG, or WEBP up to 50MB</p>
              )}
            </div>
          </div>
        </div>
